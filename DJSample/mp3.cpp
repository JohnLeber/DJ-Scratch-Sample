...
/*
    ...
    */
HRESULT CMP3::OpenFromFile(TCHAR* inputFileName) {
    // Open the mp3 file
...
void CMP3::SetBufferPtr(BYTE** ppSoundBuffer, DWORD& dwBufferLength)
{
    *ppSoundBuffer = soundBuffer;
...
*/
HRESULT CMP3::OpenFromMemory(BYTE* mp3InputBuffer, DWORD mp3InputBufferSize) {
    ...
    mp3Assert(wmHeaderInfo->GetAttributeByName(&wmStreamNum, L"Bitrate", &wmAttrDataType, (BYTE*)&m_dwBitrate, &lengthDataType));
    ...
    bufferLength = durationInSecondInt * pcmFormat.nAvgBytesPerSec;
    ...
    while (1) {
        ...
        ULONG count;
        MMRESULT mresult = mp3Stream->Read(mp3BlockBuffer, MP3_BLOCK_SIZE, &count);
        if (mresult != 0)
        {
            ATLASSERT(0);
            long nert = 0;
...
/*
    ...
    */
double __inline CMP3::GetDuration() {
